...
namespace Phoenix.Mobile.Core.Proxies.Common
{
    ...
    public class ProductSKUProxy : BaseProxy, IProductSKUProxy
    {
        ...
        public async Task<BaseResponse<ProductImageDto>> GetProductImages(ProductSKURequest request)
        {
            ...
            {
                var api = RestService.For<IProductSKUApi>(GetHttpClient());
                return await api.GetProductImages(request);
            }
            catch (Exception ex)
            {
                ExceptionHandler.Handle(new NetworkException(ex), true);
        ...
        public interface IProductSKUApi
        {
            ...
            [Get("/productSKU/GetProductImages")]
            Task<BaseResponse<ProductImageDto>> GetProductImages([Body] ProductSKURequest request);
        }
    }
}