    ...
    public class RegisterPageModel : BasePageModel
    {
        ...
        private async Task CreateUserExecute()
        {
            try
            {
                ...
                var data1 = await _userService.GetAllUsers(userRequest1);
                foreach (var item in data1)
                {
                    if (UserName == item.UserName)
                    {
                        await _dialogService.AlertAsync("Tên người dùng đã tồn tại");
                        IsBusy = false;
                        return;
                    }
                }
        ...
        public UserRequest userRequest1 { get; set; } = new UserRequest();
