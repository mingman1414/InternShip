...
namespace Phoenix.Mobile.PageModels.Common
{
    public class RatingPageModel : BasePageModel
    {
        ...
        public RatingPageModel(IDialogService dialogService, IRatingService ratingService, IOrderService orderService, IWorkContext workContext, ICustomerService orderDetailService)
        {
        ...
        private async Task LoadData()
        {
            ...
            UserId = token.UserId;
            CusRequest.zUser_Id = UserId;
            var data = await _customerDetailService.GetCustomerApptById(CusRequest);
            if (data == null)
            {
        ...
        private async Task AddRatingExecute()
        {
            try
            {
                ...
                request.ProductSKU_Id = OrderDetail.ProductSKU_Id;
                var data2 = await _ratingService.UpdateRating(request);
                IsBusy = false;
            }
            catch (Exception e)
            {
        ...
        public CustomerRequest CusRequest { get; set; } = new CustomerRequest();
