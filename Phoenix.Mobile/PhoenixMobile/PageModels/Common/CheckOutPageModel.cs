...
namespace Phoenix.Mobile.PageModels.Common
{
    public class CheckOutPageModel : BasePageModel
    {
        ...
        #region properties
        public bool ReturnAddOrder { get; set; }
        ...
        private async Task AddOrderExecute()
        {
            try
            {
                ...
                {
                    if (item.Quantity > item.Warehouseq)
                    {
                        await _dialogService.AlertAsync("Sản phẩm " + item.ProductName + " không đủ số lượng để bán");
                        IsBusy = false;
                        return;
                    }
                }
            ...
            await _dialogService.AlertAsync("Đặt hàng thành công");
        }
        #endregion
    }
}
