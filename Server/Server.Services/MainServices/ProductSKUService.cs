...
namespace Phoenix.Server.Services.MainServices
{
    public interface IProductSKUService
    {
        ...
        Task<BaseResponse<ProductImageDto>> GetProductImages(ProductSKURequest request);
    }
    ...
    public class ProductSKUService : IProductSKUService
    {
        ...
        public async Task<BaseResponse<ProductSKUDto>> GetAllProductSKUById(int id, ProductSKURequest request)
        {
            ...
            try
            {
                ...
                query = query.Where(d => d.Deleted.Equals(request.Deleted ));
                query = query.Where(d => d.Product_Id.Equals(id));
        ...
        public async Task<BaseResponse<ProductSKUAppDto>> GetProductById(ProductSKURequest request)
        {
            ...
            try
            {
                var query = (from p in _dataContext.Products
                             ...
                             join w in _dataContext.Warehouses on s.Id equals w.ProductSKU_Id
                             select new
                             {
                                 Id = s.Id,
                                 Warehouseq = w.Quantity,
        ...
        #region GetProductByImage
        public async Task<BaseResponse<ProductImageDto>> GetProductImages(ProductSKURequest request)
        {
            var result = new BaseResponse<ProductImageDto>();
            try
            {
                var query = (from p in _dataContext.Products
                             join s in _dataContext.ProductSKUs on p.Id equals s.Product_Id
                ...
                var query2 = (from p in _dataContext.Products
                              join s in _dataContext.ProductSKUs on p.Id equals s.Product_Id
                              join i in _dataContext.ImageRecords on p.Image2 equals i.Id
                              select new
                              {
                                  Id = i.Id,
                                  Product_Id = s.Product_Id,
                ...
                var query3 = (from p in _dataContext.Products
                              join s in _dataContext.ProductSKUs on p.Id equals s.Product_Id
                              join i in _dataContext.ImageRecords on p.Image3 equals i.Id
                              select new
                              {
                                  Id = i.Id,
                                  Product_Id = s.Product_Id,
                ...
                var query4 = (from p in _dataContext.Products
                              join s in _dataContext.ProductSKUs on p.Id equals s.Product_Id
                              join i in _dataContext.ImageRecords on p.Image4 equals i.Id
                              select new
                              {
                                  Id = i.Id,
                                  Product_Id = s.Product_Id,
                ...
                var query5 = (from p in _dataContext.Products
                              join s in _dataContext.ProductSKUs on p.Id equals s.Product_Id
                              join i in _dataContext.ImageRecords on p.Image5 equals i.Id
                              select new
                              {
                                  Id = i.Id,
                                  Product_Id = s.Product_Id,
                ...
                var data1 = query.Where(d => d.Product_Id == request.Product_Id).Union(query2).Where(x => x.Product_Id == request.Product_Id).Union(query3).Where(w => w.Product_Id == request.Product_Id).Union(query4).Where(c => c.Product_Id == request.Product_Id).Union(query5).Where(a => a.Product_Id == request.Product_Id);

                var config = new MapperConfiguration(cfg => cfg.CreateMissingTypeMaps = true);
                var mapper = config.CreateMapper();
                var listcart = data1.Select(mapper.Map<ProductImageDto>).ToList();
                // var data = listcart.First(d => d.Id == request.Id);
                result.Data = listcart.MapTo<ProductImageDto>();
            }
            catch (Exception ex)
            {
